...
当前开发版本为 `0.1.0-alpha.50`。P0、离线标准可信链、用户触发/二次确认的标准在线升级客户端、未部署服务端固定协议与本地真实签名纵向链、统一账号/SyncRecord、桌面 PKCE/加密会话、账号/设备绑定的 Ed25519 权益客户端、独立服务端签发、规范化订阅事件与属主设备列表/撤销 API 源码、网站同步历史与订阅/掩码设备管理客户端，以及网站撤销后桌面显式刷新降级的匿名纵向链已经实现；compatible 三类只读 AI 建议源码也已实现，Ollama 与 LM Studio 各一个固定组合通过窄范围匿名验收。账号、权益与标准更新受信配置仍为 `pending_configuration`，生产私钥/公钥、支付商 webhook 适配、真实标准发布源、迁移/API/数据库/网站部署和官方云 AI 未完成。最新真实 Windows 制品仍为未签名 alpha.42，不是可售卖正式版；macOS、真实安装生命周期、代码签名、公证、来源/许可人工签署和 OS 级隔离仍是门禁。普通 build/test 永不联网；任何重新下载仍须用户明确批准。具体事实必须以代码、`npm test` 和 `docs/DEVELOPMENT_STATUS.md` 交叉核对。

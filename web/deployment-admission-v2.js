// Fail-closed deployment admission for the direct-object Web data plane.
// Declared capabilities are configuration evidence only, never production proof.
...
const trackedRequirements = require("./deployment-requirements-v2.json");
...
const PUBLIC_HTTP_KEYS = Object.freeze([
  "max_control_request_bytes", "max_control_response_bytes", "max_execution_ms",
...
const OBJECT_STORAGE_KEYS = Object.freeze([
  "max_object_bytes", "supports_private_bucket", "supports_presigned_put",
  "supports_presigned_get", "supports_presigned_expiry_control",
  "supports_exact_origin_cors", "supports_strong_consistency",
  "supports_conditional_create", "supports_source_etag_copy", "supports_metadata",
  "supports_head", "supports_paginated_prefix_list", "supports_delete_confirmation",
]);
...
function validateRequirements(value) {
  ...
  if (value.schema_version !== "2.0" ||
      value.requirements_type !== "oak_manuscript_web_direct_deployment_requirements") {
    throw new TypeError("Web 直传部署要求身份不兼容");
  }
  const expectedSources = [
    "web/direct-transfer-contract.js",
    "web/supabase-s3-direct-storage.js",
    "web/persistent-job-service.js",
    "web/python-core-process-processor.js",
    "web/supabase/005_direct_object_transfer.sql",
  ...
  if (!Array.isArray(value.source_contracts) ||
    ...
    throw new TypeError("Web 直传部署要求来源集合漂移");
  }
  exactObject(value.public_http, [
    "min_control_request_bytes", "min_control_response_bytes", "min_execution_ms",
  ...
  exactObject(value.object_storage, [
    "min_object_bytes", "requires_private_bucket", "requires_presigned_put",
    "requires_presigned_get", "requires_presigned_expiry_control",
    "requires_exact_origin_cors", "requires_strong_consistency",
    "requires_conditional_create", "requires_source_etag_copy", "requires_metadata",
    "requires_head", "requires_paginated_prefix_list", "requires_delete_confirmation",
  ], "Web 直传对象存储要求");
  ...
  if (value.public_http.min_control_request_bytes !== DEFAULT_MAX_JSON_BYTES ||
      value.public_http.min_control_response_bytes !== DEFAULT_MAX_JSON_BYTES ||
      value.private_execution.min_execution_ms !== DEFAULT_PROCESS_TIMEOUT_MS ||
      value.object_storage.min_object_bytes !== DEFAULT_MAX_RESULT_BYTES) {
    throw new TypeError("Web 直传部署要求与当前运行时上限漂移");
  }
...
const DIRECT_DEPLOYMENT_REQUIREMENTS_SHA256 = createHash("sha256")
  .update(canonicalBytes(REQUIREMENTS))
  .digest("hex");

function validateProfile(input) {
  const profile = exactObject(input, TOP_LEVEL_KEYS, "Web 直传平台 profile");
  if (profile.schema_version !== "2.0" ||
      profile.profile_type !== "oak_manuscript_web_direct_platform_profile") {
    throw new TypeError("Web 直传平台 profile 身份不兼容");
  }
  if (typeof profile.profile_id !== "string" || !PROFILE_ID_PATTERN.test(profile.profile_id)) {
    throw new TypeError("profile_id 不是规范的非敏感标识");
  }
  const groups = {
    public_http: exactObject(profile.public_http, PUBLIC_HTTP_KEYS, "public_http"),
    private_execution: exactObject(profile.private_execution,
      PRIVATE_EXECUTION_KEYS, "private_execution"),
    object_storage: exactObject(profile.object_storage, OBJECT_STORAGE_KEYS, "object_storage"),
    database: exactObject(profile.database, DATABASE_KEYS, "database"),
    operations: exactObject(profile.operations, OPERATIONS_KEYS, "operations"),
  };
...
function assessDirectWebDeploymentProfile(input) {
  ...
  addCapacity(profile.public_http.max_control_request_bytes,
    REQUIREMENTS.public_http.min_control_request_bytes, "CONTROL_REQUEST_BYTES_INSUFFICIENT");
  addCapacity(profile.public_http.max_control_response_bytes,
    REQUIREMENTS.public_http.min_control_response_bytes, "CONTROL_RESPONSE_BYTES_INSUFFICIENT");
  addCapacity(profile.public_http.max_execution_ms,
    REQUIREMENTS.public_http.min_execution_ms, "CONTROL_EXECUTION_WINDOW_INSUFFICIENT");
  ...
  addCapacity(profile.object_storage.max_object_bytes,
    REQUIREMENTS.object_storage.min_object_bytes, "OBJECT_BYTES_INSUFFICIENT");
  for (const [key, code] of [
    ["supports_private_bucket", "PRIVATE_BUCKET_UNSUPPORTED"],
    ["supports_presigned_put", "PRESIGNED_PUT_UNSUPPORTED"],
    ["supports_presigned_get", "PRESIGNED_GET_UNSUPPORTED"],
    ["supports_presigned_expiry_control", "PRESIGNED_EXPIRY_CONTROL_UNSUPPORTED"],
    ["supports_exact_origin_cors", "EXACT_ORIGIN_CORS_UNSUPPORTED"],
    ["supports_strong_consistency", "STRONG_CONSISTENCY_UNSUPPORTED"],
    ["supports_conditional_create", "CONDITIONAL_CREATE_UNSUPPORTED"],
    ["supports_source_etag_copy", "SOURCE_ETAG_COPY_UNSUPPORTED"],
    ["supports_metadata", "OBJECT_METADATA_UNSUPPORTED"],
    ["supports_head", "OBJECT_HEAD_UNSUPPORTED"],
    ["supports_paginated_prefix_list", "PREFIX_LIST_UNSUPPORTED"],
    ["supports_delete_confirmation", "DELETE_CONFIRMATION_UNSUPPORTED"],
  ]) addCapability(profile.object_storage[key], code);
  for (const [group, pairs] of [
    [profile.database, [
      ["supports_transactions", "TRANSACTIONS_UNSUPPORTED"],
      ["supports_advisory_locks", "ADVISORY_LOCKS_UNSUPPORTED"],
      ["supports_row_level_security", "RLS_UNSUPPORTED"],
      ["supports_service_role_rpc", "SERVICE_ROLE_RPC_UNSUPPORTED"],
    ]],
    [profile.operations, [
      ["supports_private_worker_scheduler", "PRIVATE_WORKER_SCHEDULER_UNSUPPORTED"],
      ["supports_cleanup_scheduler", "CLEANUP_SCHEDULER_UNSUPPORTED"],
      ["supports_retry_alerting", "RETRY_ALERTING_UNSUPPORTED"],
      ["supports_secret_injection", "SECRET_INJECTION_UNSUPPORTED"],
    ]],
  ]) {
    for (const [key, code] of pairs) addCapability(group[key], code);
  }
  return deepFreeze({
    schema_version: "2.0",
...
function getDirectWebDeploymentRequirements() {
  return REQUIREMENTS;
}

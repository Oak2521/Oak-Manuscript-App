...
} = require("./standards-revocation-service");
...
const ERROR_MESSAGES = Object.freeze({
  ...
  CREDENTIALS_NOT_ALLOWED: "标准撤回获取不接受账号凭据",
  INSECURE_TRANSPORT: "标准撤回获取只接受 HTTPS",
  INVALID_HEADERS: "请求头非法",
  INVALID_JSON: "JSON 请求体非法",
  INVALID_REQUEST: "标准撤回请求不符合 exact v1 契约",
  LENGTH_REQUIRED: "标准撤回请求必须声明 Content-Length",
  METHOD_NOT_ALLOWED: "该路由只接受 POST",
  NOT_FOUND: "API 路由不存在",
  REQUEST_TOO_LARGE: "标准撤回请求超过上限",
  SERVICE_UNAVAILABLE: "标准撤回服务暂时不可用",
...
function fail(code) { throw new StandardsRevocationHttpError(code); }
...
function declaredLength(request) {
  ...
  if (value === undefined) fail("LENGTH_REQUIRED");
...
function sendEnvelope(response, bytes) {
...
function normalizeError(error) {
  if (error instanceof StandardsRevocationHttpError) return error.code;
  if (error instanceof StandardsRevocationServiceError && Object.hasOwn(ERROR_STATUS, error.code)) {
    return error.code;
  }
  return "SERVICE_UNAVAILABLE";
}

function validateStandardsRevocationHttpErrorResponse(value) {
  if (!exactKeys(value, ["schema_version", "error", "request_id"]) ||
    ...
    throw new TypeError("标准撤回 HTTP error 不符合 exact v1 契约");
  }
  return true;
}

function validateStandardsRevocationHttpAuditEvent(value) {
  if (!exactKeys(value, [
    "schema_version", "event_type", "request_id", "occurred_at", "method", "route",
    "http_status", "error_code",
  ]) || value.schema_version !== "1.0" ||
      value.event_type !== "standards_revocation_http_request_completed" ||
      !UUID_PATTERN.test(value.request_id || "") || !METHOD_PATTERN.test(value.method || "") ||
      ![REVOCATION_API_PATH, "unmatched"].includes(value.route) ||
    ...
    throw new TypeError("标准撤回 HTTP audit 不符合 exact v1 契约");
  }
  return true;
}

function createStandardsRevocationHttpHandler({
  service,
  expectedOrigin,
  ...
  if (!service || typeof service.fetch !== "function") {
    throw new TypeError("标准撤回 service 未实现 fetch");
  }
  canonicalOrigin(expectedOrigin);
  if ([isSecureRequest, requestIdFactory, clock, securityEventSink]
    .some((item) => typeof item !== "function")) {
    throw new TypeError("标准撤回 HTTP handler 适配器必须是函数");
  }
  return async function handleStandardsRevocationRequest(request, response) {
    ...
    try {
      ...
      if (request.url !== REVOCATION_API_PATH) fail("NOT_FOUND");
      route = REVOCATION_API_PATH;
      if (method !== "POST") fail("METHOD_NOT_ALLOWED");
      if (singleHeader(request, "authorization") !== undefined ||
          singleHeader(request, "cookie") !== undefined) {
        fail("CREDENTIALS_NOT_ALLOWED");
      }
      if (singleHeader(request, "accept") !== REVOCATION_MEDIA_TYPE ||
      ...
      const result = await service.fetch(input);
      if (!result || !Buffer.isBuffer(result.envelopeBytes) ||
          Object.keys(result).sort().join("\0") !== ["envelopeBytes"].join("\0")) {
        fail("SERVICE_UNAVAILABLE");
      }
      status = 200;
      sendEnvelope(response, result.envelopeBytes);
    } catch (error) {
      ...
      validateStandardsRevocationHttpErrorResponse(body);
      ...
      try {
        ...
        const event = {
          schema_version: "1.0",
          event_type: "standards_revocation_http_request_completed",
          request_id: requestId,
        ...
        validateStandardsRevocationHttpAuditEvent(event);

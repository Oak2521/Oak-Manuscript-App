...
- 离线生成 alpha.54 Windows x64 NSIS、ZIP 与 win-unpacked；真实 ASAR/production identity、9 项 fuse、packaged 资源门禁、双进程加密队列恢复和强制 EpubCheck/Ace 隐藏 smoke 均通过；
- 首次最终发布证据因 release 根存在 alpha.42 残留而按设计拒绝；旧文件保留到版本化 archive 后，alpha.54 schema v2 SHA/manifest/smoke evidence 才生成并交叉复验；
- alpha.54 对归档 alpha.12 的安装生命周期只读预检通过；未启动安装器或改动系统。NSIS/EXE 仍为 `NotSigned`，packaged sale 仍有 12 blocker、发行身份缺 12 字段。
...
- 桌面标准联网配置升为 exact 1.1：release/revocation 端点必须同时配置、同源、规范 HTTPS 且使用两个固定路径；待配置状态二者必须同时为 null，main 仅在两个客户端都成功构造后原子启用；
- 用户一次点击固定先获取、验签并原子应用撤回清单，成功后才检查 release；撤回服务失败、配置不完整或状态漂移均停止后续查询，仍无后台联网；
- active 被撤回时普通操作继续阻断，标准内容与 release 摘要停止展示，但经过验证的恢复状态仍可启用同一入口；安全前进候选使用专门告警和一次性确认，安装成功后恢复 ready；
- renderer/preload 不暴露端点、原始签名 bytes 或安装权限；固定请求保持内容无关，不含稿件、项目、账号、设备或结果；
- 相关专项 35/34/0/1；全量 Node 697/690/0/7、Python 362/0/0/3；资源信任 108 文件 / 2,171,922 字节，manifest `2012cc68…58b1`、anchor `0634a9e9…56f6`；隐藏 Electron source smoke PASS；
- 正式端点、生产 trust pin/密钥治理、真实发布源、部署、监控和紧急手册仍未完成；销售门禁仍 17 项，发行身份仍缺 12 字段。
...
- 新增 exact `desktop-standards-update` 配置与 HTTPS 客户端：只有完整 `configured` 才建立 transport；一次用户点击只发送 APP 版本、标准 bundle、发布序列和 manifest 摘要，不发送稿件、项目路径或账号；
- 候选 `.oakstd` 原始字节限制为 24 MiB，拒绝重定向、Cookie、压缩、媒体类型漂移、超长与异常上游正文；下载后沿用 Ed25519 签名、canonical manifest、哈希、schema、allowlist、兼容性与防降级验证；
- 主进程生成 10 分钟、一次性的内存安装计划并显示原生集中确认；渲染层拿不到更新地址、候选字节或 plan ID。取消、超时、并发、重放、状态漂移和安装失败均保留旧版本；
- 安装只切换新建项目的默认标准，已有项目继续固定原版本，仍须查看差异并另行确认，不会静默升级；默认 `pending_configuration` 保证普通运行零标准更新网络；
- 新增/相关专项 28 项：27 通过、0 失败、1 条件跳过；全量回归 Node 666/659/0/7、Python 362/0/0/3；资源信任 98 文件 / 2,159,692 字节，manifest `9fbc2d02…0103`，anchor `d2dca027…f558`；Web 与 Electron 隐藏源码 smoke PASS；
- 生产 trust pin、真实更新地址、服务端发布/撤回、真实网络联调仍未完成；销售资源门禁仍有 17 项，发行身份仍缺 12 个字段。

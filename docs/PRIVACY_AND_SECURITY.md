...
> 当前权威为商业正式版开发方案 v2.0；本文件描述 `0.1.0-alpha.57` 源码隐私边界。标准治理摘要只含条目分类计数和门禁布尔值，不含稿件、账号、端点或密钥；Web 能力 profile 同样不能转化为生产证据。默认账号/权益无网络目标，仓库无生产私钥；最新 alpha.54 Windows 制品仍未签名。真实账号、支付、迁移、API/官网部署和生产隔离未实现。

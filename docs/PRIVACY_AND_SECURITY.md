...
> 当前权威为商业正式版开发方案 v2.0；v1.2 仅作历史基线。本文件描述 `0.1.0-alpha.6` 源码检查点已实现的桌面隐私与安全边界。当前没有 alpha.6 安装包或 ZIP。新增的 Windows builder 下载器是开发者显式运行的构建输入工具，不在 APP/default session 中运行、不读取稿件，并且本轮未实际联网。商业方案中的 Web 上传、统一账号、订阅与同步尚未实现，必须在各自阶段另行完成威胁建模和验收。

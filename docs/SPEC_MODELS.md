...
## 9. Web 临时作业模型（alpha.28 契约、HTTP、GoTrue、内容存储、持久状态与私有 worker 边界）
...
状态机、handler、Bearer/GoTrue/Fetch、Netlify Blobs 适配器、Supabase/Postgres 持久源码、service-role-only 原子领取和本机固定 Python 子进程都不是已部署生产服务。processor 只接收最小文档枚举与 Buffer，不接收账号、任务 ID 或租约；完成仍绑定不可复制服务内句柄的 exact lease/revision/expiry。真实数据库迁移与 RLS/多实例验证、生产容器/OS 禁网和资源隔离、恶意文件门禁、短时下载及零留存审计仍待实现。

...
- [x] Markdown 围栏代码、行内代码、表格和强制换行尾随空格排除；短行密集的诗歌/刻意排版块保守豁免；
- [x] TXT/Markdown 问题位置包含行号；结果页及 JSON/Markdown/HTML 报告显示不含稿件内容的格式覆盖矩阵；
- [x] BOM、CRLF/LF、空文件、排除上下文和源稿 SHA-256 不变均有测试；标准 release 2.1.0 / sequence 3 及 v1→v2→v3 历史 CAS 验证通过；
- [x] Node 719/712/0/7、Python 368/0/0/3，源码/Web/packaged 隐藏 smoke、Windows packaged fuse/资源和 schema v2 发布证据通过；
- [ ] Windows 代码签名、真实安装生命周期、macOS 双架构资源/构建/签名/公证和生产联网验收完成；当前 alpha.58 仍是未签名内测包。

## 0.1.0-alpha.57 标准治理状态透明化验收（2026-07-29）

- [x] 治理摘要只从已通过 release/payload 验证的标准数组派生；重复 ID、未知来源类型、审阅状态或核验状态失败关闭；
- [x] 摘要精确报告 13 项、active 9 / under_review 4、verified 0 / pending 12 / unavailable 1，门禁为 false；
- [x] 标准页将审阅状态与来源核验分列，不再把 active 简写为“有效”，并明确阻止“完整标准库”表述；
- [x] 撤回或治理摘要异常时清空旧表格，不保留陈旧成功状态；动态内容只用文本节点渲染；
- [x] 聚焦 75/74/0/1、Node 716/709/0/7、Python 362/0/0/3、标准/资源门禁和独立隐藏 Electron smoke 通过；
- [ ] 13 项标准的来源核验、具名审校签核和覆盖补全完成；当前 0 verified、4 under_review，仍不满足正式发行；
- [ ] alpha.57 Windows/macOS 制品、签名、公证和真实安装生命周期完成；最新 Windows packaged 仍为未签名 alpha.54。

## 0.1.0-alpha.56 Web 平台能力准入验收（2026-07-29，历史）
...
- [x] 一次明确点击严格按“恢复状态核验 → 获取/验签/原子应用撤回清单 → 检查 release”执行；撤回阶段失败时 release lookup 为 0，默认无后台联网；
- [x] active 撤回后普通业务继续阻断，标准内容和 release 摘要不展示；经过验证的恢复状态仍可启用按钮，安全前进候选使用专门原生告警并绑定一次性计划；
- [x] UI/preload 不获得端点、原始签名清单或安装权限；请求继续不含账号、设备、项目、稿件、路径或结果；
- [x] 相关专项 35/34/0/1；全量 Node 697/690/0/7、Python 362/0/0/3；资源、标准、fuse、Electron runtime、Windows alpha 与发行身份结构门禁通过；隐藏 Electron source smoke 通过；
- [ ] 生产 release/revocation 端点和 trust pin、真实发布存储、密钥托管/多人签署、告警/监控、紧急手册、真实网络与部署未完成；alpha.53 未打包，最新 Windows 制品仍为未签名 alpha.42。

...
> 状态：`0.1.0-alpha.53` 已实现本地验签/状态语义、未部署的内容无关 service、固定 HTTP/Fetch 契约、有界桌面客户端，以及同源 exact 桌面配置和“先验撤回、后查更新”的 main/IPC/UI 安全恢复入口。生产发布源、正式端点/trust pin、调度、密钥托管和真实网络尚未接线。

## 目的与边界

标准包更新签名证明“这个候选由发布角色授权”，不能证明它之后没有因密钥泄露、内容错误或其他严重问题被撤回。撤回清单因此使用独立的 `revocation` 信任角色；发布服务或普通网络响应不能自行把某个 manifest 标成已撤回。

清单只包含标准库身份和 manifest SHA-256，不含账号、设备、项目、稿件、文件名、路径或检查结果。alpha.52—alpha.53 的网络形状与桌面接线测试使用进程内 Fetch 适配器、注入客户端和运行时生成的 Ed25519 密钥；没有真实联网、生产公钥或生产签名。
...
桌面 `StandardsRevocationHttpClient` 只接受上述固定 HTTPS 路径，10 秒默认超时，响应上限 1 MiB，拒绝重定向、凭据、Cookie、压缩、范围响应、媒体/长度漂移和异常正文。`StandardsProvider.refreshRemoteRevocations()` 再调用本地独立角色验签和追加式原子事务；并发刷新拒绝。

alpha.53 的 `desktop-standards-update` exact 1.1 配置同时要求同源 `/manuscript/standards/v1/revocations` 与 `/manuscript/standards/v1/check`；pending 时二者必须同时为 null。main 只在两个 client 均成功构造后同时交给 Provider。一次明确点击严格先刷新撤回、再检查 release；第一步失败就停止。active 被撤回后，标准列表返回不含内容的恢复视图，普通业务仍阻断，但按钮可在两个 transport 和生产 trust 同时有效时寻找安全前进版本。当前 tracked 配置仍为 pending，因此普通 APP 运行不会发出请求。

## 当前证据与未完成项

alpha.51 测试覆盖 exact Schema、canonical JSON、时间窗、bundle、排序/去重、未知 key、阈值、签名、追加式防回退、active fail-closed、已撤回候选拒绝、检查途中并发撤回优先、安全前进恢复、禁止回滚、CAS/历史报告不变以及状态提交失败恢复。alpha.52 新增 11 项，覆盖公开 exact 请求、发布源/摘要投毒、HTTPS/framing/凭据/媒体门禁、有界响应、超时/异常净化、并发刷新，以及真实测试签名清单从假服务到桌面验签和原子应用。alpha.53 增加双端点 exact 配置、main 原子启用、撤回优先 IPC 顺序、失败短路、被撤回内容隐藏、恢复入口与恢复状态竞态有界失败测试；相关三文件共 35 total / 34 pass / 1 条件跳过。

以下仍未完成：生产 revocation 公钥 pin、正式双端点配置、离线密钥保管与多人签署、发布审批、真实发布存储、客户端调度/缓存、紧急撤回操作手册、告警/监控和真实网络联调。main/IPC/UI 源码已经接线，但不能作为“生产撤回系统已上线”的证据。

...
标准页有“检查在线更新”“安装本地标准包”和“回滚全局标准”入口。当前内置 2.0.0，严格签名/CAS/回滚/项目 pin 验证已实现；旧项目只能从本地 CAS 中仍存在且已验证的原 release 迁移，不会用最新包冒充。正式生产 trust pin 与更新地址仍未配置，因此两个更新入口默认禁用，当前源码也没有已部署更新服务。未来配置完成后，APP 仍只在您点击检查时联网，并在验签后再次集中确认；不会后台自动检查或静默改变已有项目。

alpha.51 已实现独立签名撤回清单的本地安全语义；alpha.52 已验证未部署假服务到桌面的内容无关获取链，但主进程配置、用户入口、调度和告警界面尚未接线，当前普通 APP 不会自动联网获取。未来若受信清单撤回当前标准，APP 将停止新的检查、修复和报告重新生成，直到安装更高且未撤回的签名标准；不会删除项目、旧检查结果、标准 CAS 或已经生成的导出文件。用户仍可使用“打开导出目录”访问既有文件；已撤回的上一版本不能成为回滚目标。
...
验证器会按源码 `package.json` 的当前版本读取 EXE/ZIP，核对 PE/ZIP 结构、单链接文件身份、字节数与 SHA-256，再交叉验证 SHA 文件和 canonical manifest；不会扫描其他版本替代当前制品。alpha.42 的 NSIS、ZIP、SHA 文件与 schema v2 canonical manifest 已通过交叉验证；当前源码已升 alpha.52 且尚无对应制品，因此运行 alpha.52 发布验证应按设计拒绝缺失，不能复用 alpha.42 字节。

安装生命周期验收器默认只读、不启动安装器，并要求当前源码版本存在精确制品。alpha.42 对归档 alpha.12 的只读预检已经通过；当前 alpha.48 未构建，因此不具备 alpha.48 预检输入：

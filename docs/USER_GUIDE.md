...
**便携包**：解压 `湖岸稿件 Oak Manuscript-x.y.z-win.zip`，双击 `湖岸稿件 Oak Manuscript.exe`。
无需安装 Python 或任何依赖（运行时已内置）。首次运行 Windows SmartScreen 可能提示
「未知发布者」（内部开发版未签名），选择「仍要运行」。

**开发运行**：`npm install` 后 `npm start`；UI 冒烟自检 `npm run smoke`。

流程：欢迎页（隐私说明）→ 选稿件或匿名样本 → 选项目目录 → 选检查目标 → 检查 →
问题页逐条 接受/拒绝/暂不处理，可「自动修复（仅白名单）」→ 导出中心（修订稿、三种报告、
PDF 样张、基础 EPUB 预览、脱敏评估摘要）→ 验证完整性。

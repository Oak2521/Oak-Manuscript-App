...
alpha.51 已实现独立签名撤回清单的本地安全语义，但生产清单获取和告警界面尚未接线。未来若受信清单撤回当前标准，APP 将停止新的检查、修复和报告重新生成，直到安装更高且未撤回的签名标准；不会删除项目、旧检查结果、标准 CAS 或已经生成的导出文件。用户仍可使用“打开导出目录”访问既有文件；已撤回的上一版本不能成为回滚目标。
...
验证器会按源码 `package.json` 的当前版本读取 EXE/ZIP，核对 PE/ZIP 结构、单链接文件身份、字节数与 SHA-256，再交叉验证 SHA 文件和 canonical manifest；不会扫描其他版本替代当前制品。alpha.42 的 NSIS、ZIP、SHA 文件与 schema v2 canonical manifest 已通过交叉验证；当前源码已升 alpha.48 且尚无对应制品，因此运行 alpha.48 发布验证应按设计拒绝缺失，不能复用 alpha.42 字节。

...
**订阅权益（当前边界）**：设置页显示当前 Free/Pro、有效/宽限/过期/撤销状态。查看状态不联网；只有已登录用户点击“刷新订阅权益”才会向未来配置的固定湖岸端点请求一次签名权益。客户端必须验证 Ed25519 签名及账号、设备、issuer、audience 和时间，成功后才更新 OS 加密缓存；失败只降为 Free，不锁已有项目。alpha.48 匿名纵向链证明网站撤销不会静默改写桌面旧缓存，用户再次刷新后桌面取得真实签名 revoked 权益并安全降级；当前权益配置仍为 `pending_configuration`，生产私钥和真实服务不存在，网站页面也未部署，因此最终用户目前仍不能在线使用这些能力。
...
标准页有“检查在线更新”“安装本地标准包”和“回滚全局标准”入口。当前内置 2.0.0，严格签名/CAS/回滚/项目 pin 验证已实现；旧项目只能从本地 CAS 中仍存在且已验证的原 release 迁移，不会用最新包冒充。正式生产 trust pin 与撤回/更新地址仍未配置，因此两个更新入口默认禁用，当前源码也没有已部署更新服务。未来配置完成后，APP 仍只在您点击检查时联网：先取得并验证撤回清单，成功后才检查更新候选，再以原生窗口集中确认；不会后台自动检查或静默改变已有项目。

alpha.51 已实现独立签名撤回清单的本地安全语义；alpha.52 已验证未部署假服务到桌面的内容无关获取链；alpha.53 已把撤回优先顺序和恢复告警接入主进程与标准页。若受信清单撤回当前标准，APP 会停止新的检查、修复和报告重新生成，并隐藏已撤回标准的内容，但在正式双端点和 trust pin 有效时保留“检查在线更新”作为安全恢复入口；只有更高且未撤回的签名标准可以恢复工作。撤回不会删除项目、旧检查结果、标准 CAS 或已经生成的导出文件。用户仍可使用“打开导出目录”访问既有文件；已撤回的上一版本不能成为回滚目标。当前 tracked 配置仍为空，因此普通 APP 不会联网获取。
...
验证器会按源码 `package.json` 的当前版本读取 EXE/ZIP，核对 PE/ZIP 结构、单链接文件身份、字节数与 SHA-256，再交叉验证 SHA 文件和 canonical manifest；不会扫描其他版本替代当前制品。alpha.42 的 NSIS、ZIP、SHA 文件与 schema v2 canonical manifest 已通过交叉验证；当前源码已升 alpha.53 且尚无对应制品，因此运行 alpha.53 发布验证应按设计拒绝缺失，不能复用 alpha.42 字节。

安装生命周期验收器默认只读、不启动安装器，并要求当前源码版本存在精确制品。alpha.42 对归档 alpha.12 的只读预检已经通过；当前 alpha.53 未构建，因此不具备 alpha.53 预检输入：

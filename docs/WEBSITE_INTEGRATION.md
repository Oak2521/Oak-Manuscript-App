...
## Provider 一览（当前 alpha.7）

alpha.7 新增的发布制品证据生成器及 alpha.6 的联网下载器均仅属于开发/发布工具，与运行时 Provider、稿件检查和网站对接完全隔离；它们没有改变 Provider 的网络行为。统一账号、订阅、同步和 Web 作业仍未实现。未来的同步白名单可包含最终体例、解析模式、原因码、置信度和解析器版本，但不得包含引用/书目原文、姓名或内容哈希。

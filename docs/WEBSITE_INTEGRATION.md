...
## Provider 一览（当前 alpha.6）

alpha.6 新增的联网下载器仅供开发者在用户明确授权后取得固定 Windows builder 归档，与运行时 Provider、稿件检查和网站对接完全隔离；它没有改变 Provider 的网络行为。统一账号、订阅、同步和 Web 作业仍未实现。未来的同步白名单可包含最终体例、解析模式、原因码、置信度和解析器版本，但不得包含引用/书目原文、姓名或内容哈希。

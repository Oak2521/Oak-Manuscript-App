...
## Provider 一览（当前 alpha.11）

alpha.11 继承 alpha.8 已固定的账号、权益和结果同步离线客户端契约，但没有生产认证、计费、持久队列或网络 transport。alpha.11 的 ASAR 资源锚点、Ace utility/loopback Chrome 与 fuse 验证器、alpha.7 的发布证据生成器及 alpha.6 的联网下载器都与账号/同步 Provider 和网站对接隔离；资源验证不联网，Ace loopback 仅为本机进程控制。本轮未联网。SyncRecord v1 可包含最终体例、解析模式、原因码、置信度和解析器版本，但不得包含引用/书目原文、姓名、路径或内容哈希；权威字段见 `SYNC_RECORD_V1.md` 和 `config/schemas/sync-record-v1.schema.json`。

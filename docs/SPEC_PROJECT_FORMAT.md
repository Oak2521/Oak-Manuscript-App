...
- `sync.history` 在当前 `0.1.0-alpha.33` 仍为空数组。本机 `pending_transport` 队列独立存放在 OS 加密的应用 userData 中，不伪造项目内同步成功历史；Web 临时作业、HTTP handler 与账号适配器也不写入项目同步历史。生产服务端回执上线时必须先版本化本节格式、提供兼容读取，并只在服务端确认后写成功记录。

...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.19`；已有未签名 Windows x64 NSIS/ZIP 与真实 packaged 证据，不代表正式发行
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.19 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.19 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.19 packaged PASS）** | 保持 Electron sandbox，在外层隐藏 GUI 进程完成 DOCX + EPUB 全闭环并强制运行 EpubCheck/Ace；各 4 次检查、1 个批次、3 个检查点且原稿哈希不变 |
...
| Windows NSIS / ZIP | **完成（未签名 alpha）** | 三归档验哈希导入；builder 独立全树锁成立；alpha.19 NSIS/ZIP 已生成并复验 |
| Windows 发布制品证据 | **完成（alpha.19）** | SHA256SUMS + canonical manifest 与真实 NSIS/ZIP 交叉复验通过 |
...
| ASAR 资源信任根 | **真实 packaged 验证完成** | ASAR 锚点固定 72 个应用 loose 文件及四份平台锁；启动前全树复核；真实 packaged 关闭 5 个 loose 可信根 blocker |
| 发行商/销售主体元数据 | **契约与门禁完成，真实身份待确认** | 已知产品/品牌/appId/官网固定；法定主体、支持/隐私/条款链接、版权、签名主体和具名复核显式待定；alpha 列 blocker，sale fail-closed |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 155.2 秒**；Node 355/348/0/7（3.210 秒），Python 351/0 failures/0 errors/3 skipped（103.702 秒）。跳过项不计作通过。
- 发行身份专项纳入 Node 全量：11 项通过；当前仓库身份结构有效但 `complete=false`，12 个 Windows 完备性字段显式缺失；重复键、字段/顺序/schema/canonical 字节、占位文本、非官方 URL、平台身份和 package 漂移均 fail-closed。
- 资源信任专项已纳入 Node 全量：源码清单/锚点一致、loose 应用与四类运行资源漂移、额外文件/平台替换、真实 `app.asar` 读取和启动顺序均通过；`verify:resource-trust` PASS，72 文件 / 2,115,011 字节的应用清单 SHA-256 `480c0e3092ae4ebd94d5d554f03bdafa492c2c7adcbc4e1476d82b96b949e669`，锚点 SHA-256 `2fc7f8df3ed2559eea3cc9aab6fbe5d25113895cb5619e1975ebee348f69cb87`。
...
- alpha.19 packaged smoke 强制应用内外部验证，不再依赖调用者设置环境变量；缺陷 EPUB 的 EpubCheck/Ace 均产生可解析失败结果。保持 Electron sandbox，在获准外层隐藏进程取得 PASS，未使用 `--no-sandbox` 作为证据。
...
- 外层隐藏 `npm run smoke:packaged:win`：**SMOKE-RESULT PASS**；最新输出 `out/packaged-smoke/runs/ms4wb5l6-92a65d90b8504698/projects/`。两个项目均为 `app_version=0.1.0-alpha.19`、`source_hash_ok=true`、4 次检查、1 个修复批次、3 个检查点；当前问题 13/5、应用 issue fixes 5/2、PDF 251,663/178,404 字节；EPUB EpubCheck 为 5 error，Ace 为 8 项失败断言。
- 当前测试环境：Windows 11，Python 3.14.6，Node 24.16.0，npm 11.13.0，Electron 43.1.0，Java 21.0.11。
- Windows alpha 资源门禁：**PASS**。
  - 应用 loose 资源：72 个文件 / 2,115,011 字节；ASAR 锚点与源码清单一致；
...
- 真实 alpha.19 `app.asar`、9 项 fuse、loose 全树、Python/JRE/EpubCheck/Ace、发行身份、CPython/EpubCheck/Temurin-JRE/Electron/builder provenance 与应用 smoke 已验证；packaged 资源中 `.pyc` 为 0。
- 最终 NSIS 189,985,848 字节 / SHA-256 `9fc35cbf…9561`；ZIP 233,802,099 字节 / `1641678b…3d99`；发布证据复验通过。
...
- `windows_install_acceptance.js` 默认只读验证当前 alpha.19 与归档 alpha.12 的 release manifest、SHA256SUMS、大小、摘要和 PE；实际系统变更必须同时提供 `--run --allow-system-mutation`；
...
- Windows Authenticode 和安装包签名尚未完成；alpha.19 制品仅供开发/内测；
- alpha.19 已将 electron-builder 的 `package.json` 缺 `author` 警告升级为 `RELEASE_PUBLISHER_METADATA_PENDING`；法定销售主体、正式 URL、版权、发行者、具名复核与签名证书主体尚未确认，不能自行猜填；

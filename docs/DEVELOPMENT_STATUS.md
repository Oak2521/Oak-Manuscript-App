...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.38`（将在本轮提交后建立）；本轮是 SyncRecord 服务端/API/Supabase 与未接线桌面 transport 源码检查点，没有部署或重新打包；最新真实 Windows 制品与 packaged 证据仍为 alpha.37
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.38 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.38 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.38 source；最新 packaged 为 alpha.37）** | alpha.38 保持 Electron sandbox，在独立隐藏进程完成既有全闭环；alpha.37 packaged 双启动和哈希绑定证据继续有效，但不能冒充 alpha.38 制品 |
...
| Windows NSIS / ZIP | **完成（未签名 alpha）** | 三归档验哈希导入；builder 独立全树锁成立；alpha.37 NSIS/ZIP 已生成并复验 |
| Windows 发布制品证据 | **完成（alpha.37 schema v2）** | SHA256SUMS + canonical manifest 与真实 NSIS/ZIP 交叉复验；manifest 另绑定 packaged-smoke 证据、EXE 和匿名输出树摘要 |
| Windows 安装生命周期 | **工具/alpha.37 预检完成，真实运行待授权** | 九阶段编排、证据 v1、HKCU/快捷方式探针和专项测试完成；历史 release manifest v1 与当前 v2 均严格验证；alpha.37/alpha.12 安装器只读预检通过；未执行真实安装、升级、回装探测或卸载 |
| Electron ASAR / fuse 硬化 | **真实制品 9 项全验** | `@electron/fuses 2.1.3` 识别 `WasmTrapHandlers`；afterPack 严格写入全部 9 项并回读；真实 EXE 无未知 fuse |
| ASAR 资源信任根 | **alpha.38 源码锁完成；alpha.37 packaged 已验证** | alpha.38 源码锚点固定 81 个应用 loose 文件及四份平台锁；最新真实 packaged 仍固定 alpha.37 的 79 文件树 |
...
| 湖岸统一账号 / Free+Pro / 结果同步 | **源码纵向 transport/service 完成，生产未接通** | 已有 PKCE 状态、Free/Pro、SyncRecord 明确预览、safeStorage 队列，以及固定桌面 Bearer client/coordinator、独立服务验证、同源 API、Supabase repository/002 迁移源码；生产 Auth 无 token、main 未实例化，未迁移/部署，无支付或网站后台 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 110.517 秒**；Node 560/553/0/7（3.8298105 秒），Python 362/0 failures/0 errors/3 skipped（102.371 秒）。跳过项不计作通过。Sync 服务/API/repository/runtime 与桌面 client/coordinator 纳入全量。
- 源码 Electron smoke：在独立隐藏进程运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5kbrfu-69765feff8e3381c/projects/`。普通 main 未实例化 Sync transport、生产 Auth 无 token，未联网。最新 packaged smoke 仍为 alpha.37。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.38 源码 `verify:resource-trust` PASS，81 文件 / 2,142,090 字节，应用清单 SHA-256 `fbc0ca36bcb670156a34769d743607590062878f583c6edc7dfcb66d37130ab2`，锚点 SHA-256 `f90b54f365293c6386135d1bf7daf28637131c1731146227b9e189a8bddd0b87`。最新真实 packaged identity/ASAR 证据仍为 alpha.37 的 79 文件版本。
...
- 账号/同步专项：既有 Auth/Free+Pro/明确确认/safeStorage 队列继续通过；alpha.38 新增独立服务端 validator、owner/幂等/容量/快照列表/删除、同源 HTTP、GoTrue/runtime、四个 Supabase RPC，以及桌面 client/coordinator。六组新增专项 **37/37**；token 错绑在 transport 前拒绝。所有网络/DB 为注入仿真或 SQL 静态检查，生产 main 仍未接线。
- Web 客户端/Fetch/GoTrue/HTTP/Supabase/作业状态机/Netlify 存储/持久 repository/私有 worker/双清扫与 SyncRecord 服务合计 **130/130 全通过**。`production_zero_retention_verified=false`；DB/store/network 仍为注入仿真或静态检查，没有真实迁移、容器、OS 禁网、平台生命周期或生产同步证明。Web 私有子包仍锁定 Blobs 10.1.0；桌面根依赖无新增。
...
- alpha.38 source smoke 强制应用内外部验证并 PASS；alpha.37 packaged smoke 继续提供第二进程队列恢复和制品哈希证据。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据；不能把不同版本证据合并称为 alpha.38 packaged 通过。
...
- 外层隐藏 `npm run smoke:packaged:win`：**SMOKE-RESULT + SYNC-RECOVERY PASS**；最终输出 `out/packaged-smoke/runs/ms5ht9j7-67fece5b58d7c515/projects/`，运行的真实二进制为 alpha.37 `release/win-unpacked/湖岸稿件 Oak Manuscript.exe`。证据绑定 EXE SHA-256 `ff85385e…efca` 与输出树 76 文件 / 1,368,471 字节 / `f0c9d687…b334`。最终 `npm run smoke` 另在 `out/source-smoke/runs/ms5hynmq-aba639b137d62e8b/projects/` PASS。
- 当前测试环境：Windows 11，Python 3.14.6，Node 24.16.0，npm 11.13.0，Electron 43.1.0，Java 21.0.11。
- Windows alpha 资源门禁：**PASS**。
  - alpha.38 源码应用 loose 资源：81 个文件 / 2,142,090 字节；源码锚点与清单一致；最新 alpha.37 packaged 为 79 文件 / 2,139,277 字节；
...
- 真实 alpha.37 `app.asar`、生产 package identity、9 项 fuse、loose 全树、Python/JRE/EpubCheck/Ace、CPython/EpubCheck/Temurin-JRE/Electron/builder provenance、双阶段应用 smoke、加密队列恢复及 schema v2 发布证据已验证；packaged 资源中 `.pyc` 为 0。
- alpha.37 首次总构建在已经完成打包和两次 smoke 后，证据树因误拒绝合法 `.oak-project-write.lock` 退出 1；规则收窄到唯一已知项目锁且其他隐藏名继续拒绝，定向回归、packaged smoke、证据生成与独立复验随后通过。最终 NSIS 190,013,357 字节 / SHA-256 `26af70e0…e095`；ZIP 233,838,480 字节 / `e4288fbf…697d`；smoke 证据 / `a90bc1c1…80a9`。
...
- 新增主进程专用 `sync-http-client.js` / `sync-transport-coordinator.js` 及 Provider 传输结果状态；固定 HTTPS/Bearer、单项单在途、账号稳定和远端 created/replayed 后精确本地删除。生产 Auth token 与 main 实例化仍故意缺席，普通 APP 不联网；
...
- `windows_install_acceptance.js` 默认只读并绑定 `package.json` 当前版本；alpha.37 对归档 alpha.12 的预检已通过，`authorized=false`。实际系统变更仍必须同时提供 `--run --allow-system-mutation`；
...
按 v2.0 方案继续，不重做总体规划；近期优先闭合用户可感知的“账号 → 权益 → 检查 → 明确同步”纵向链：

1. 在不联网、无真实凭据条件下实现生产形状的系统浏览器 PKCE 回调/状态/安全 token-store 边界及主进程接线；未配置时继续 fail-closed；
2. 把 alpha.38 coordinator 接入 main 与明确的发送/重试 UI，保持默认 session 离线、Renderer 无 token/URL/负载构造权，并补崩溃恢复/账号切换测试；
3. 真实 Supabase 迁移、GoTrue/RLS/多实例 E2E 和网站后台属于生产环境操作，需用户另行授权及有效配置后再执行；
...
- 本机加密同步队列、SyncRecord 服务/API/Supabase 与未接线桌面 transport、Web 临时作业/Blobs/Postgres/worker/双清扫源码均已实现；生产 PKCE token、main 接线、真实迁移/容器、OS 禁网、平台恶意软件扫描、调度/告警、订阅、真实零留存与官网后台仍涉及生产系统，网站保持只读；

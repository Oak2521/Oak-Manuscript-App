...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.28`；本轮为源码/SQL 静态契约/本机隔离子进程/离线仿真检查点，最新未签名 Windows x64 NSIS/ZIP 与真实 packaged 证据仍属于 alpha.23
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.28 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.28 最终计数见下方“最新测试基线”；0 失败 |
...
| Web 服务端统一处理 | **私有领取与本机共享核心纵向闭环完成，生产未部署** | 状态机、HTTPS/Origin、GoTrue、Fetch、工作台、Netlify Blobs、Postgres RLS/RPC/CAS/墓碑、`SKIP LOCKED` 私有领取、身份最小化 worker 与固定 Python `web-check` 已实现；真实迁移/容器 E2E、OS 禁网、恶意文件门禁、计费、短时下载、结果同步与官网部署待实现 |
| 可售卖正式版发布 | **未达到** | 缺跨端产物、生产账号/支付、条款、签名、公证、内测和网站联调 |

## 最新测试基线

- 最终统一 `npm test`：**PASS，退出码 0，墙钟 110.2 秒**；Node 462/455/0/7（3.528 秒），Python 352/0 failures/0 errors/3 skipped（102.047 秒）。跳过项不计作通过。
...
- ASAR/资源信任专项已纳入 Node 全量：packaged 身份从真实 `app.asar/package.json` 读取；raw header、精确短读循环、同路径重建、loose 源码伪造、ASAR 内身份漂移、源码清单/锚点、四类运行资源和启动顺序均通过。`verify:resource-trust` PASS，78 文件 / 2,126,802 字节，应用清单 SHA-256 `d11dd1eb46069ce2c06ce506c5e0f3146c02e14223913944236a572ca58696b1`，锚点 SHA-256 `85b39f8dd69ec0e9ab6cf8bfbff8420e06d20f110d605d089703680ddceb9212`。
...
- Web 客户端/Fetch/GoTrue/HTTP/Supabase/作业状态机/Netlify 存储/持久 repository/私有 worker 合计 **91 项全通过**。私有队列/处理器/存储/SQL 组合专项 31/31，覆盖 `FOR UPDATE SKIP LOCKED`、完整租约窗、processing 必有 lease、输入强一致读取、身份/任务/租约不进入 processor、不可复制句柄、失败后过期接管、固定 Python 参数/环境/scratch/超时/输出/源哈希/清理，以及真实本机 TXT 共享核心烟测。DB/store/network 仍为注入仿真或静态检查；没有连接生产 Netlify/Supabase，也没有容器或 OS 级禁网证明。Web 私有子包精确锁定 Blobs 10.1.0，alpha.26 生产依赖审计为 0 个已知漏洞；桌面根依赖无新增。
...
- Windows alpha 资源门禁：**PASS**。
  - 应用 loose 资源：78 个文件 / 2,126,802 字节；ASAR 锚点与源码清单一致；
...
- `windows_install_acceptance.js` 默认只读并绑定 `package.json` 当前版本；alpha.28 无制品，当前不会通过预检。最近一次成功证据为 alpha.23 对归档 alpha.12；实际系统变更仍必须同时提供 `--run --allow-system-mutation`；
...
- Web 参考实现已有标准 Fetch 边界、GoTrue 验证器、未部署工作台、Netlify Blobs 内容适配器、Supabase/Postgres 持久任务/幂等迁移、service-role-only 私有领取、身份最小化 worker 和固定 Python 共享核心子进程；内存状态机只保留为参考/测试。SQL 尚未在真实 Supabase 执行，本机进程隔离不是生产容器/OS 禁网证明，也没有恶意文件门禁或计费；Blobs metadata 不是平台原生 TTL，必须调度双清扫器。
...
6. 在现有本机加密同步队列、Web 作业/Blobs、持久数据库及私有 worker 源码上继续生产账号/同步、恶意文件门禁、容器/OS 禁网与资源隔离、短时下载、真实零留存、macOS 与官网嵌入；真实 Supabase 迁移与 E2E 需另行环境授权。
...
- 本机加密同步队列、Web 作业状态机、同源 HTTP handler、GoTrue verifier、Fetch 适配器、未部署工作台、Netlify Blobs 内容适配/清扫、Postgres 持久任务、私有原子领取和固定 Python 子进程共享核心已实现；真实迁移/容器配置、OS 禁网、恶意文件门禁、订阅、网络同步 transport、真实零留存与官网发布仍涉及生产系统，网站保持只读；

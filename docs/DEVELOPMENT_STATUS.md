...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.32`；本轮为源码/独立隐藏 Electron/离线 AI 设置检查点，最新未签名 Windows x64 NSIS/ZIP 与真实 packaged 证据仍属于 alpha.23
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.32 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.32 最终计数见下方“最新测试基线”；0 失败 |
...
| 三模式 AI / 用户自带 AI | **桌面设置与加密凭据完成，模型 transport 未实现** | 无 AI/湖岸 AI/我的 AI、六类供应商、Pro 门禁、safeStorage、HTTPS/loopback、换绑禁复用、凭据不回读/不同步及无静默回退已固定；没有模型请求、建议预览、Web 会话凭据或生产湖岸 AI |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 117.636 秒**；Node 492/485/0/7（3.573 秒），Python 357/0 failures/0 errors/3 skipped（109.596 秒）。跳过项不计作通过。AI/IPC/UI/存储定向 26/26；Web 既有 104 项继续纳入 Node 全量。
- 源码 Electron smoke：受限运行器先因 GPU 子进程 `0xC0000135` 退出；在独立隐藏、非受限 Electron 环境重跑 **PASS**。日志确认同步加密队列和 AI 加密设置就绪、两类 transport 均关闭；该证据不是 packaged smoke。
...
- ASAR/资源信任专项已纳入 Node 全量：packaged 身份从真实 `app.asar/package.json` 读取；raw header、精确短读循环、同路径重建、loose 源码伪造、ASAR 内身份漂移、源码清单/锚点、四类运行资源和启动顺序均通过。`verify:resource-trust` PASS，79 文件 / 2,136,323 字节，应用清单 SHA-256 `012f9bc6fcce4a330d618b33e475405cf52b16aa6adcca5f7bae10f2fef3a3c7`，锚点 SHA-256 `58d24d83e1d045a0cf26eca46202adfaf98e6a109760d82b7af52dcadf651758`。
...
- `windows_install_acceptance.js` 默认只读并绑定 `package.json` 当前版本；alpha.32 无制品，当前不会通过预检。最近一次成功证据为 alpha.23 对归档 alpha.12；实际系统变更仍必须同时提供 `--run --allow-system-mutation`；

...
- 源码检查点标签：`chatgpt-v0.1.0-alpha.10`，只标记源码与本地验证状态；尚无同版本安装包或 ZIP
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.10 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.10 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.10 PASS）** | 独立隐藏 Electron 完成 DOCX + EPUB 全闭环；EPUB 还真实运行 EpubCheck/Ace；两项目均 4 次检查、1 次批量修复、3 个检查点且原稿哈希不变 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 119.4 秒**；Node 295/288/0/7（2.461 秒），Python 351/0 failures/0 errors/3 skipped（112.121 秒）。跳过项不计作通过。
...
- alpha.10 没有重跑独立 `OAK_TEST_ACE=1` Python 慢测套件；取而代之的是更强的真实隐藏 Electron UI 证据：应用通过主进程受控 Chrome 与 utilityProcess 实际运行缺陷样本，EpubCheck/Ace 均产生可解析的失败结果。
...
- `npm run release:evidence:verify:win` 在真实 `release/` 上按预期退出 1，明确缺 `Oak-Manuscript-0.1.0-alpha.10-Windows-x64.exe`；`verify:packaged:fuses:win` 也拒绝缺失的 `release/win-unpacked`，没有生成伪证据。
- 本轮未执行联网下载或 `build:win`；真实工具树和 tracked lock 仍缺失，构建包装器会在 electron-builder 启动前 fail-closed；没有 alpha.10 安装包或 ZIP，也未运行真实 packaged fuse 验证。
...
3. 生成 alpha.10 NSIS + ZIP，逐个执行真实二进制 fuse、打包后资源门禁、版本/packaged 身份断言和包含 EpubCheck/Ace 的完整 smoke；成功后由流水线生成并复验 SHA256SUMS + canonical release manifest；
...
- Ace 已脱离开发树依赖并使用固定 utility helper，但仍依赖用户系统 Chrome；真实 packaged 验证、自带浏览器、OS 级默认拒绝网络及可信根加固未完成；
...
- Windows Authenticode 和安装包签名尚未完成；alpha.10 没有安装包或 ZIP；

...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.39`（将在本轮提交后建立）；本轮是桌面 PKCE、OS 加密 token-store 与显式同步主进程接线源码检查点，没有联网、部署或重新打包；最新真实 Windows 制品与 packaged 证据仍为 alpha.37
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.39 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.39 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.39 source；最新 packaged 为 alpha.37）** | alpha.39 保持 Electron sandbox，在独立隐藏进程完成既有全闭环；alpha.37 packaged 双启动和哈希绑定证据继续有效，但不能冒充 alpha.39 制品 |
...
| ASAR 资源信任根 | **alpha.39 源码锁完成；alpha.37 packaged 已验证** | alpha.39 源码锚点固定 84 个应用 loose 文件及当前完整平台锁；最新真实 packaged 仍固定 alpha.37 的 79 文件树 |
...
| 湖岸统一账号 / Free+Pro / 结果同步 | **桌面生产形状接线完成，生产未配置/未联调** | 已有系统浏览器 PKCE S256/state、safeStorage token-store、深链、账号绑定 access-token provider、显式发送 UI、固定桌面 client/coordinator、独立服务/API/Supabase 源码；默认端点全部为空，未做真实 PKCE/迁移/部署/支付/网站后台 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 113.8 秒**；Node 576/569/0/7（4.0589638 秒），Python 362/0 failures/0 errors/3 skipped（105.091 秒）。跳过项不计作通过。桌面 PKCE/token-store/main 条件接线与 Sync 服务/client/coordinator 均纳入全量。
- 源码 Electron smoke：在独立隐藏进程运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5kxdpe-fa5aab63ad422c0f/projects/`。默认账号配置没有端点或 key，未联网。最新 packaged smoke 仍为 alpha.37。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.39 源码 `verify:resource-trust` PASS，84 文件 / 2,145,925 字节，应用清单 SHA-256 `9b1a292bb58ac8ae021691c37c877af288efc6ea043dbec10628bc9681e5d313`，锚点 SHA-256 `a5504168689213f4a4219c4aac3104d88ec10d24fb73eebf3985e07b8c02f160`。最新真实 packaged identity/ASAR 证据仍为 alpha.37 的 79 文件版本。
...
- 账号/同步专项：既有 Free+Pro/明确确认/safeStorage 队列与 alpha.38 服务/client/coordinator 继续通过；alpha.39 新增待配置零网络、PKCE S256/state、严格深链、加密 token-store、身份复核、条件 main 和逐项发送测试。所有网络/DB 为注入仿真或 SQL 静态检查，受信端点仍为空。
...
- alpha.39 source smoke 强制应用内外部验证并 PASS；alpha.37 packaged smoke 继续提供第二进程队列恢复和制品哈希证据。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据；不能把不同版本证据合并称为 alpha.39 packaged 通过。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.39 源码应用 loose 资源：84 个文件 / 2,145,925 字节；源码锚点与清单一致；最新 alpha.37 packaged 为 79 文件 / 2,139,277 字节；
...
1. 补完账号/同步异常注入：token exchange/refresh 模糊失败、浏览器打开失败、并发回调、进程崩溃恢复和远端成功后本地删除失败；默认配置继续 fail-closed；
2. 真实 Supabase 迁移、GoTrue/RLS/多实例 E2E、正式 OAuth/OIDC 契约和网站后台属于生产环境操作，需用户另行授权及有效配置后再执行；未知协议下不得宣称 nonce/ID-token 验证已完成；

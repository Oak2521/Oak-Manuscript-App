...
- Web 客户端/Fetch/GoTrue/HTTP/Supabase/作业状态机/Netlify 存储/持久 repository/私有 worker 合计 **94 项全通过**。Python 上传门禁专项 5/5，Node 门禁/持久/HTTP 定向 35/35；覆盖路径逃逸、链接/特殊文件、加密/重复成员、展开/压缩比/CRC、宏/ActiveX/嵌入/DDE、EPUB 脚本、格式伪装、UTF-8/NUL、拒绝零入库与身份最小化。DB/store/network 仍为注入仿真或静态检查；该结构门禁不是病毒库扫描，也没有容器或 OS 级禁网证明。Web 私有子包精确锁定 Blobs 10.1.0，alpha.26 生产依赖审计为 0 个已知漏洞；桌面根依赖无新增。

...
- 当前源码本地标签：`chatgpt-v0.1.0-alpha.49-standards-update-transport`；`chatgpt-v0.1.0-alpha.48-revoke-refresh-e2e` 为跨端撤销检查点，`chatgpt-v0.1.0-alpha.42-packaged` 为最新 Windows 打包证据
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.49 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.49 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.49 source；alpha.42 packaged）** | alpha.49 源码与最新 alpha.42 win-unpacked 均保持 Electron sandbox 并完成隐藏本地闭环；packaged 双启动、加密队列恢复和哈希绑定证据属于 alpha.42 |
...
| ASAR 资源信任根 | **alpha.49 源码、alpha.42 packaged 已验证** | alpha.49 源码锚点固定 98 个 loose 文件；真实 `app.asar`、packaged 全树与发布证据仍对应 alpha.42，未混称同一制品 |
...
| 标准包验证、升级与回滚 | **桌面受控链完成；生产服务未配置** | 内置 2.0.0；canonical manifest、签名/CAS/高水位/回滚、项目七字段 pin、差异确认、升级后强制重检均已实现；alpha.49 增加用户点击检查、有界下载、验签后二次确认和原子安装。默认无地址/trust pin；服务端发布/撤回与真实联网未实现 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 110.7 秒**；Node 666/659/0/7（3.862 秒），Python 362/0 failures/0 errors/3 skipped（102.404 秒）。跳过项不计作通过。新增标准在线升级配置/transport/provider/IPC/UI 及既有客户端/服务/账号/同步/AI 链均纳入全量。
...
- Web 客户端 smoke：alpha.49 实际 `web/client/index.html` 在隐藏 Chromium 中使用匿名内存假服务完成桌面/移动布局与设备撤销，**PASS**；HTTP(S) 请求 0、撤销前有效按钮 1、撤销后 0、完整设备 ID 未出现在页面。截图为 `out/web-client-smoke/desktop.png` 与 `mobile.png`；这不是生产账号/API/部署证据。
- 源码 Electron smoke：alpha.49 最终源码在独立隐藏窗口运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5ww99a-62e72c834adc1ea4/projects/`，未使用 `--no-sandbox`。默认标准更新配置为空，因此 smoke 未产生标准更新网络；也未配置账号、权益或 AI。最新 alpha.42 packaged smoke 也通过，见下。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.49 源码 `verify:resource-trust` PASS，98 文件 / 2,159,692 字节，应用清单 SHA-256 `9fbc2d02267027fc1cc707768f45d8010e9ded2f4a25fe55bf859c7bd4a10103`，锚点 SHA-256 `d2dca0272b9d574ca2f7f715bf04d310839f658f58fce12157e2366071f16558`；真实 packaged `app.asar` 证据仍为 alpha.42。
...
- alpha.49 source 与最新 alpha.42 packaged smoke 均强制应用内外部验证并 PASS；packaged 第二进程队列恢复、制品/输出树哈希证据只属于 alpha.42。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.49 源码 loose 资源：98 个文件 / 2,159,692 字节，manifest `9fbc2d02…0103`、anchor `d2dca027…f558`；最新真实 ASAR/packaged 全树仍为 alpha.42；
...
1. alpha.49 已完成用户触发的标准更新 transport、候选验签、一次性计划与原生确认闭环；下一项在不联网前提下实现固定服务端发布响应契约与假服务端到桌面的 E2E，保持默认零网络和项目版本不静默升级；
...
- 标准包生产 trust pin、真实服务端发布/撤回和联网联调尚未实现；alpha.49 桌面检查/下载客户端已完成，但默认地址为空，本地签名包导入仍因无生产 trust pin 按设计禁用；

...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.37`；本轮生成并复验未签名 Windows x64 NSIS/ZIP、packaged 资源/ASAR/9-fuse、哈希绑定 smoke 证据、schema v2 发布摘要及安装生命周期只读预检
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.37 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.37 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.37 source + packaged 双启动 PASS）** | 保持 Electron sandbox，在外层隐藏进程完成 DOCX + EPUB 全闭环、EpubCheck/Ace、加密队列写入及第二进程恢复；packaged 结果另有哈希绑定证据；原稿哈希不变 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 165.2 秒**；Node 523/516/0/7（4.1144228 秒），Python 362/0 failures/0 errors/3 skipped（113.806 秒）。跳过项不计作通过。新增 packaged-smoke 证据正反向覆盖；AI 网络底座与 Web 既有测试继续纳入 Node 全量。
- 源码 Electron smoke：按既有方式在独立隐藏外层进程运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5hynmq-aba639b137d62e8b/projects/`。真实完成 AI 单条问题发送预览/transport 缺席取消零发送，并继续完成 DOCX/EPUB 原闭环；HTTP client 未实例化、未联网。
...
- ASAR/资源信任专项已纳入 Node 全量：packaged 身份从真实 `app.asar/package.json` 读取；raw header、精确短读循环、同路径重建、loose 源码伪造、ASAR 内身份漂移、源码清单/锚点、四类运行资源和启动顺序均通过。`verify:resource-trust` PASS，79 文件 / 2,139,277 字节，应用清单 SHA-256 `4ce4810d54f180d961f644b8f5d66e7b3aba6996e1a0c5c64b75397c93ab1b97`，锚点 SHA-256 `4f306d10d385c8b913b03782a8672eb66022096bab836bffed5bb9ed027bbf92`。
...
- Web 客户端/Fetch/GoTrue/HTTP/Supabase/作业状态机/Netlify 存储/持久 repository/私有 worker/双清扫协调器合计 **104 项全通过**。清扫报告只含计数，阶段失败不阻断后续清扫，扫描上限与截断显式可见；`production_zero_retention_verified=false`。DB/store/network 仍为注入仿真或静态检查；结构门禁不是病毒库扫描，也没有容器、OS 级禁网或生产平台生命周期证明。Web 私有子包精确锁定 Blobs 10.1.0，alpha.26 生产依赖审计为 0 个已知漏洞；桌面根依赖无新增。
...
- alpha.37 source 与 packaged smoke 均强制应用内外部验证和第二进程队列恢复，不依赖调用者设置环境变量；缺陷 EPUB 的 EpubCheck/Ace 均产生可解析失败结果。保持 Electron sandbox，在获准外层隐藏进程取得 PASS，未使用 `--no-sandbox` 作为证据。
...
2. Ace 的来源/全闭包许可、自带浏览器、OS 级网络隔离和代码签名/可信见证仍待完成；alpha.37 已有本地哈希绑定 smoke 证据，但它不是不可伪造证明；
...
6. 在现有本机加密同步队列、Web 作业/Blobs、持久数据库、上传结构门禁、私有 worker、一次性领取及本地有界双清扫源码上继续受控生产调度/告警、生产账号/同步、平台恶意软件扫描、容器/OS 禁网与资源隔离、真实零留存、macOS 与官网嵌入；真实 Supabase 迁移与 E2E 需另行环境授权。

...
- 当前源码本地标签：`chatgpt-v0.1.0-alpha.47-web-license-account`；`chatgpt-v0.1.0-alpha.46-subscription-devices` 为设备服务检查点，`chatgpt-v0.1.0-alpha.42-packaged` 为最新 Windows 打包证据
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.47 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.47 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.47 source；alpha.42 packaged）** | alpha.47 源码与最新 alpha.42 win-unpacked 均保持 Electron sandbox 并完成隐藏本地闭环；packaged 双启动、加密队列恢复和哈希绑定证据属于 alpha.42 |
...
| ASAR 资源信任根 | **alpha.47 源码、alpha.42 packaged 已验证** | alpha.47 源码锚点固定 96 个 loose 文件；真实 `app.asar`、packaged 全树与发布证据仍对应 alpha.42，未混称同一制品 |
...
| 湖岸统一账号 / Free+Pro / 结果同步 | **客户端、签发、订阅事件、设备服务与网站账号界面生产形状完成，生产未配置/未联调** | 已有系统浏览器 PKCE/加密会话、桌面显式同步、SyncRecord 服务/API/Supabase 源码；alpha.44 完成桌面签名权益和网站同步历史，alpha.45 完成独立签发，alpha.46 完成规范化订阅事件与设备服务，alpha.47 完成网站订阅状态、掩码设备列表与逐台确认撤销客户端。默认配置为空且无生产私钥；支付商 webhook 验签适配、真实 PKCE/迁移/API/部署未完成 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟约 111 秒**；Node 654/647/0/7（4.091 秒），Python 362/0 failures/0 errors/3 skipped（102.510 秒）。跳过项不计作通过。网站订阅/设备客户端、规范化订阅事件、属主撤销、HTTP/runtime 以及既有签发/账号/同步/AI 链均纳入全量。
...
- Web 客户端 smoke：alpha.47 实际 `web/client/index.html` 在隐藏 Chromium 中使用匿名内存假服务完成桌面/移动布局与设备撤销，**PASS**；HTTP(S) 请求 0、撤销前有效按钮 1、撤销后 0、完整设备 ID 未出现在页面。截图为 `out/web-client-smoke/desktop.png` 与 `mobile.png`；这不是生产账号/API/部署证据。
- 源码 Electron smoke：alpha.47 当前源码在独立隐藏窗口运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5vaowt-9d9cd8ae2b355871/projects/`，未使用 `--no-sandbox`。smoke 未配置账号、权益或 AI。最新 alpha.42 packaged smoke 也通过，见下。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.47 源码 `verify:resource-trust` PASS，96 文件 / 2,158,481 字节，应用清单 SHA-256 `098e520b793a041036313fbda532f41cd435d732a2ba7cb03840b4f0093e3e8f`，锚点 SHA-256 `d9bf27a96d61ffe5d30a077a280ebfa7349f3d6a836367c02bdcf7f04d2af7f9`；真实 packaged `app.asar` 证据仍为 alpha.42。
...
- alpha.47 source 与最新 alpha.42 packaged smoke 均强制应用内外部验证并 PASS；packaged 第二进程队列恢复、制品/输出树哈希证据只属于 alpha.42。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.47 源码 loose 资源：96 个文件 / 2,158,481 字节，manifest `098e520b…e8f`、anchor `d9bf27a9…7f9`；最新真实 ASAR/packaged 全树仍为 alpha.42；
...
1. alpha.47 已完成网站账号后台订阅状态、掩码设备列表和逐台确认撤销客户端；下一项串起账号 → 订阅状态 → 网站撤销设备 → 桌面刷新权益的无真实秘密仓库内纵向测试；
2. 纵向测试必须使用真实服务端签名与桌面验签/缓存状态机，证明撤销后桌面降为 Free、已有本地项目不锁定；真实 Supabase/GoTrue/OAuth/OIDC、迁移和网站联调仍需另行授权及有效预生产配置；

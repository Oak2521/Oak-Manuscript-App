...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.33`；本轮为源码/独立隐藏 Electron/离线 AI 发送预览检查点，最新未签名 Windows x64 NSIS/ZIP 与真实 packaged 证据仍属于 alpha.23
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.33 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.33 最终计数见下方“最新测试基线”；0 失败 |
...
| 三模式 AI / 用户自带 AI | **桌面设置、加密凭据与单条问题发送预览完成，模型 transport 未实现** | 无 AI/湖岸 AI/我的 AI、六类供应商、Pro/safeStorage/端点边界已固定；`ai-context`、10 分钟一次性计划、完整发送披露、上下文/配置过期拒绝、注入式建议只读返回已实现；生产 transport=`null`，没有真实模型请求、Web 会话凭据或湖岸 AI 服务 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 116.771 秒**；Node 501/494/0/7（3.561 秒），Python 362/0 failures/0 errors/3 skipped（108.900 秒）。跳过项不计作通过。AI/IPC/UI 定向 35/35，Python AI 上下文定向 5/5；Web 既有 104 项继续纳入 Node 全量。
- 源码 Electron smoke：扩展链路首次因未剥离 CLI `ok` 字段被 exact validator 拒绝，修正后独立隐藏 Electron **PASS**。真实完成湖岸 AI 设置、单条问题发送预览、transport 缺席确认按钮禁用和取消零发送，并继续完成 DOCX/EPUB 原闭环；该证据不是 packaged smoke。
...
- ASAR/资源信任专项已纳入 Node 全量：packaged 身份从真实 `app.asar/package.json` 读取；raw header、精确短读循环、同路径重建、loose 源码伪造、ASAR 内身份漂移、源码清单/锚点、四类运行资源和启动顺序均通过。`verify:resource-trust` PASS，79 文件 / 2,139,277 字节，应用清单 SHA-256 `dac22358086fdc38726cebc68bca32668fbae35167967b473b367b0d9ce98388`，锚点 SHA-256 `42d749a4e62f85c87a3c0d88a6242c919da2d8c26c07ca60312359dbcd410f98`。
...
- `windows_install_acceptance.js` 默认只读并绑定 `package.json` 当前版本；alpha.33 无制品，当前不会通过预检。最近一次成功证据为 alpha.23 对归档 alpha.12；实际系统变更仍必须同时提供 `--run --allow-system-mutation`；

...
- 当前源码本地标签：`chatgpt-v0.1.0-alpha.46-subscription-devices`；`chatgpt-v0.1.0-alpha.45-entitlement-issuer` 为服务端签发检查点，`chatgpt-v0.1.0-alpha.42-packaged` 为最新 Windows 打包证据
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.46 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.46 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.46 source；alpha.42 packaged）** | alpha.46 源码与最新 alpha.42 win-unpacked 均保持 Electron sandbox 并完成隐藏本地闭环；packaged 双启动、加密队列恢复和哈希绑定证据属于 alpha.42 |
...
| ASAR 资源信任根 | **alpha.46 源码、alpha.42 packaged 已验证** | alpha.46 源码锚点固定 96 个 loose 文件；真实 `app.asar`、packaged 全树与发布证据仍对应 alpha.42，未混称同一制品 |
...
| 湖岸统一账号 / Free+Pro / 结果同步 | **客户端、签发、订阅事件与设备服务生产形状完成，生产未配置/未联调** | 已有系统浏览器 PKCE/加密会话、桌面显式同步、SyncRecord 服务/API/Supabase 源码；alpha.44 完成桌面签名权益和网站同步历史客户端，alpha.45 完成独立签发，alpha.46 完成规范化订阅事件与属主设备列表/撤销服务。默认配置为空且无生产私钥；支付商 webhook 验签适配、网站设备 UI、真实 PKCE/迁移/API/部署未完成 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 114.5 秒**；Node 648/641/0/7（4.320 秒），Python 362/0 failures/0 errors/3 skipped（104.323 秒）。跳过项不计作通过。规范化订阅事件、幂等/乱序处理、账号订阅概览、属主设备撤销、HTTP/runtime 以及既有签发/账号/同步/AI 链均纳入全量。
...
- 源码 Electron smoke：alpha.46 当前源码在独立隐藏窗口运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5u8go3-2af4409c2a05d15c/projects/`，未使用 `--no-sandbox`。smoke 未配置账号、权益或 AI，未产生网络请求。最新 alpha.42 packaged smoke 也通过，见下。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.46 源码 `verify:resource-trust` PASS，96 文件 / 2,158,481 字节，应用清单 SHA-256 `9e69054bb3ba1f1858bef00d96e3fe5031ac745504115988bce403b289eb0ce5`，锚点 SHA-256 `e3e84afc723a851b791664e614874e6e345832378aa818eade4e4df483f0f698`；真实 packaged `app.asar` 证据仍为 alpha.42。
...
- 账号/权益/同步专项：既有明确确认/safeStorage 队列、服务/client/coordinator、PKCE/加密 token-store/main 接线继续通过；alpha.45 的独立签发链之上，alpha.46 新增 provider 绑定的规范化订阅快照、canonical 指纹、重放/冲突/乱序语义、账号订阅概览、属主设备撤销、固定 HTTP/runtime 和响应投毒拒绝。所有网络/DB 仍为注入仿真或 SQL 静态检查，受信端点/公钥为空且仓库没有生产私钥。
...
- alpha.46 source 与最新 alpha.42 packaged smoke 均强制应用内外部验证并 PASS；packaged 第二进程队列恢复、制品/输出树哈希证据只属于 alpha.42。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.46 源码 loose 资源：96 个文件 / 2,158,481 字节，manifest `9e69054b…ce5`、anchor `e3e84afc…f698`；最新真实 ASAR/packaged 全树仍为 alpha.42；

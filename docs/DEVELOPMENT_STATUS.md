...
- 本地标签：`chatgpt-v0.1.0-alpha.43-lmstudio-e2e`（当前源码/LM Studio 窄验收）、`chatgpt-v0.1.0-alpha.42-packaged`（最新 Windows 打包证据）；真实 Ollama 验收为 `chatgpt-v0.1.0-alpha.42-ollama-e2e`
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.43 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.43 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.43 source；alpha.42 packaged）** | alpha.43 源码与最新 alpha.42 win-unpacked 均保持 Electron sandbox 并完成隐藏本地闭环；packaged 双启动、加密队列恢复和哈希绑定证据属于 alpha.42 |
...
| ASAR 资源信任根 | **alpha.43 源码、alpha.42 packaged 已验证** | alpha.43 源码锚点固定 84 个 loose 文件；真实 `app.asar`、packaged 全树与发布证据仍对应 alpha.42，未混称同一制品 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 114.5 秒**；Node 599/592/0/7（3.987 秒），Python 362/0 failures/0 errors/3 skipped（102.521 秒）。跳过项不计作通过。LM Studio 模型身份拒绝、空工具数组兼容、真实验收脚本及既有账号/同步链均纳入全量。
...
- 源码 Electron smoke：alpha.43 在独立隐藏进程运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5qdhf2-dd22990533467ecf/projects/`。smoke 未配置 AI、未产生模型请求。最新 alpha.42 packaged smoke 也通过，见下。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.43 源码 `verify:resource-trust` PASS，84 文件 / 2,145,925 字节，应用清单 SHA-256 `9f514ae8c4c3ecf52a213ef729359c408ef34cc3f621b8baf20830751ac1c2a6`，锚点 SHA-256 `3583f05b1e7f8b8177f45a707a695226d31f6128ab9a50ea2b996d3c4408150d`；真实 packaged `app.asar` 证据仍为 alpha.42。
...
- alpha.43 source 与最新 alpha.42 packaged smoke 均强制应用内外部验证并 PASS；packaged 第二进程队列恢复、制品/输出树哈希证据只属于 alpha.42。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.43 源码 loose 资源：84 个文件 / 2,145,925 字节，manifest `9f514ae8…c1c2a6`、anchor `3583f05b…8150d`；最新真实 ASAR/packaged 全树仍为 alpha.42；

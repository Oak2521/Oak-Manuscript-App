...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.31`；本轮为源码/本机隔离子进程/离线仿真检查点，最新未签名 Windows x64 NSIS/ZIP 与真实 packaged 证据仍属于 alpha.23
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.31 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.31 最终计数见下方“最新测试基线”；0 失败 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 153.3 秒**；Node 474/467/0/7（4.063 秒），Python 357/0 failures/0 errors/3 skipped（144.338 秒）。跳过项不计作通过。全部 Web 定向 104/104；alpha.31 清扫定向 38/38。
...
- ASAR/资源信任专项已纳入 Node 全量：packaged 身份从真实 `app.asar/package.json` 读取；raw header、精确短读循环、同路径重建、loose 源码伪造、ASAR 内身份漂移、源码清单/锚点、四类运行资源和启动顺序均通过。`verify:resource-trust` PASS，79 文件 / 2,136,323 字节，应用清单 SHA-256 `9c6fededb293bc6baa1d58035b132cbb57dcaeb203d2161110f96979cdcf1ed2`，锚点 SHA-256 `72b4b54a9849108a7432aa7f7054cb72bd39686b3ecf5612c54bab5e3f5483ab`。
...
- `windows_install_acceptance.js` 默认只读并绑定 `package.json` 当前版本；alpha.31 无制品，当前不会通过预检。最近一次成功证据为 alpha.23 对归档 alpha.12；实际系统变更仍必须同时提供 `--run --allow-system-mutation`；

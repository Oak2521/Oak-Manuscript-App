...
> 更新日期：2026-07-29。新记录在上；“已完成”必须有本地测试或构建证据。
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.57 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.57 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.57 source；alpha.54 packaged）** | alpha.57 源码与 alpha.54 win-unpacked 均保持 Electron sandbox 并完成隐藏本地闭环；packaged 证据仍只属于 alpha.54 |
...
| Windows NSIS / ZIP | **完成（未签名 alpha）** | 三归档验哈希导入；builder 独立全树锁成立；alpha.54 NSIS/ZIP 已离线生成并复验 |
| Windows 发布制品证据 | **完成（alpha.54 schema v2）** | SHA256SUMS + canonical manifest 与真实 NSIS/ZIP 交叉复验；manifest 另绑定 packaged-smoke 证据、EXE 和匿名输出树摘要 |
| Windows 安装生命周期 | **工具/alpha.54 预检完成，真实运行待授权** | 九阶段编排、证据 v1、HKCU/快捷方式探针和专项测试完成；历史 release manifest v1 与当前 v2 均严格验证；alpha.54/alpha.12 安装器只读预检通过；未执行真实安装、升级、回装探测或卸载 |
| Electron ASAR / fuse 硬化 | **真实制品 9 项全验** | `@electron/fuses 2.1.3` 识别 `WasmTrapHandlers`；afterPack 严格写入全部 9 项并回读；真实 EXE 无未知 fuse |
| ASAR 资源信任根 | **alpha.57 源码、alpha.54 packaged 已验证** | alpha.57 源码锚点固定 108 个 loose 文件；最新真实 `app.asar`、packaged 全树与发布证据仍属于 alpha.54 |
...
| 标准包验证、升级、撤回与回滚 | **桌面+服务端源码链完成；生产未配置** | 内置 2.0.0；canonical manifest、签名/CAS/高水位/回滚、项目七字段 pin、差异确认、升级后强制重检均已实现；alpha.49—alpha.50 增加更新 client/server E2E，alpha.51 增加独立角色签名撤回本地状态，alpha.52 增加固定撤回获取链，alpha.53 统一 exact 配置并接入“先撤回、后更新”的 main/IPC/UI 恢复入口。默认两个端点与 trust pin 仍为空；生产发布/撤回源与真实联网未实现 |
| 标准与规则补全 | **治理结构与用户披露完成，内容补全未完成** | 13 标准/35 规则/6 fixer 映射一致；alpha.57 UI 分列显示审阅/来源核验并阻止“完整”表述；外部来源核验仍为 0 项，4 项仍 under_review，真实审校签核与多类标准深度不足 |
| 湖岸统一账号 / Free+Pro / 结果同步 | **生产形状源码与本地匿名主流程完成，生产未配置/未联调** | alpha.44—alpha.48 已完成权益、签发、订阅/设备服务、网站客户端与撤销传播；alpha.54 贯通登录、Pro、本地结果、明确确认、即时发送/失败留队、服务端 owner 绑定与网站历史 strict parse。默认配置为空且无生产私钥；支付商 webhook、真实 PKCE/迁移/API/部署未完成 |
...
| Web 服务端统一处理 | **组合、迁移来源及平台准入完成，生产未部署** | alpha.55 增加 exact 组合与 SQL 字节清单；alpha.56 绑定 50 MiB/100 MiB/240 秒及隔离、存储、数据库、调度能力，runtime 拒绝不足 profile。厂商官方规格和真实 profile 尚未核对，readiness 仍为生产未就绪；生产账号/API/官网部署待实现 |
| 可售卖正式版发布 | **未达到** | 缺跨端产物、生产账号/支付、条款、签名、公证、内测和网站联调 |

## 最新测试基线

- 最终统一 `npm test`：**PASS，退出码 0，墙钟 147.8 秒**；Node 716/709/0/7（5.880 秒），Python 362/0 failures/0 errors/3 skipped（136.527 秒）。跳过项不计作通过。新增标准治理 5 项，既有标准 payload/provider/IPC、账号/权益、同步、AI、Web 与发布门禁均纳入全量。
...
- Web 客户端 smoke：alpha.55 实际 `web/client/index.html` 在隐藏 Chromium 中使用匿名内存假服务完成桌面/移动布局与设备撤销，**PASS**；HTTP(S) 请求 0，证据在 `out/web-client-smoke/`。该页面本轮未改，这仍不是生产账号/API/部署证据。
- 源码 Electron smoke：alpha.57 在文件系统沙箱内两次因 GPU 子进程 Windows `0xC0000135` / `ERR_FAILED` 失败，不计作通过；随后在沙箱外独立隐藏运行 **PASS**，输出 `out/source-smoke/runs/ms64dbwg-b5b0296cb99c4cf4/projects/`；应用 Renderer 仍为 sandbox，未使用 `--no-sandbox`。默认配置未产生标准、账号、权益或 AI 网络。最新 packaged smoke 仍属于 alpha.54。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.57 源码 `verify:resource-trust` PASS，108 文件 / 2,171,922 字节，应用清单 SHA-256 `a4d03367e4a3dd3cf7d0ab0173ee4f4ed4325676a10b417939ed283a7b4dc65a`，锚点 SHA-256 `9c29b17e4fe4d12959c56437768d84762c546a7c970d51367724df3fea65df75`；最新真实 packaged `app.asar` 仍为 alpha.54。
...
- Web 链继续全通过；alpha.56 的部署需求 SHA-256 为 `6f993e8abd88b2df9e5964638901f453708c9b6f56280b96fb0149045a27eb77`，5 项准入专项与 3 项组合专项共 8/8。能力 profile 不含秘密；合格自报仍固定 `production_evidence_verified=false`、`production_ready=false`。真实 DB/store/network 和厂商 profile 均未运行/核对。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.56 源码 loose 资源：108 个文件 / 2,171,922 字节，manifest `2b783bc5…1bad3`、anchor `948f1582…f7095`；最新真实 ASAR/packaged 全树证据仍属于 alpha.54；
...
按 v2.0 方案继续，不重做总体规划；alpha.56 已补齐 Web 平台能力准入，近期直接推进生产联调前置：

1. 经用户联网授权后，以官方当前文档核对候选平台的请求/响应/时限/子进程/调度/存储能力，生成具来源的真实 profile；未经核对不得用测试 profile 选择部署拓扑；
2. 真实 Supabase/GoTrue/OAuth/OIDC、支付商 webhook、迁移和网站联调仍需另行授权、平台选择及有效预生产配置；得到配置后先执行隔离预生产迁移/RLS/最小作业/清扫证据，再接官网；
...
5. alpha 产物不得表述为可售卖正式版；alpha.43 本机 LM Studio/Ollama 证据与 alpha.54 packaged 哈希都不是签名、不可伪造证明或全面兼容矩阵。
...
- Windows Authenticode 和安装包签名尚未完成；alpha.54 制品仅供开发/内测；
- alpha.54 继续从真实 `app.asar/package.json` 验证发行身份，但 `author` 等缺口仍由 `RELEASE_PUBLISHER_METADATA_PENDING` 阻断；法定销售主体、正式 URL、版权、发行者、具名复核与签名证书主体尚未确认，不能自行猜填；
- 标准治理 schema、完整身份、本地升级链和 alpha.57 用户可见治理摘要已实现，但没有任何外部来源完成核验，4 项外部标准仍在审阅，reviewer 仅为角色占位，GB/T、APA、Chicago、EPUB、TXT/Markdown、纸质出版和可访问性覆盖仍不够；UI 已明确阻止“标准库完整”表述，但内容缺口本身仍未关闭；
...
- 本机加密同步队列、确认后即时发送/失败留队、SyncRecord 服务/API/Supabase、桌面 PKCE/token-store/条件 main 接线和网站历史解析均已在本地生产形状 E2E 贯通；Web 临时作业/Blobs/Postgres/worker/双清扫源码也已实现。受信账号配置仍为空，真实 OAuth/OIDC、迁移/容器、OS 禁网、平台恶意软件扫描、调度/告警、订阅、真实零留存与官网后台仍涉及生产系统，网站保持只读；

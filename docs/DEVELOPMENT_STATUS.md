...
- 当前源码本地标签：`chatgpt-v0.1.0-alpha.48-revoke-refresh-e2e`；`chatgpt-v0.1.0-alpha.47-web-license-account` 为网站客户端检查点，`chatgpt-v0.1.0-alpha.42-packaged` 为最新 Windows 打包证据
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.48 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.48 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.48 source；alpha.42 packaged）** | alpha.48 源码与最新 alpha.42 win-unpacked 均保持 Electron sandbox 并完成隐藏本地闭环；packaged 双启动、加密队列恢复和哈希绑定证据属于 alpha.42 |
...
| ASAR 资源信任根 | **alpha.48 源码、alpha.42 packaged 已验证** | alpha.48 源码锚点固定 96 个 loose 文件；真实 `app.asar`、packaged 全树与发布证据仍对应 alpha.42，未混称同一制品 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 113.9 秒**；Node 655/648/0/7（4.450 秒），Python 362/0 failures/0 errors/3 skipped（103.755 秒）。跳过项不计作通过。新增网站撤销 → 桌面显式刷新纵向链及既有客户端/服务/账号/同步/AI 链均纳入全量。
...
- Web 客户端 smoke：alpha.48 实际 `web/client/index.html` 在隐藏 Chromium 中使用匿名内存假服务完成桌面/移动布局与设备撤销，**PASS**；HTTP(S) 请求 0、撤销前有效按钮 1、撤销后 0、完整设备 ID 未出现在页面。截图为 `out/web-client-smoke/desktop.png` 与 `mobile.png`；这不是生产账号/API/部署证据。
- 源码 Electron smoke：alpha.48 当前源码在独立隐藏窗口运行，应用保持 Renderer sandbox，最终 **PASS**；输出 `out/source-smoke/runs/ms5vtmj9-ac64d9c05c87e6de/projects/`，未使用 `--no-sandbox`。smoke 未配置账号、权益或 AI。最新 alpha.42 packaged smoke 也通过，见下。
...
- ASAR/资源信任专项已纳入 Node 全量：alpha.48 源码 `verify:resource-trust` PASS，96 文件 / 2,158,481 字节，应用清单 SHA-256 `267db60af4df868d2377ee8c148b9210459ebf2a9c63c89a998921c882fbfc06`，锚点 SHA-256 `e1f4718d81979793e12153e9164293b18bb092d0823b156f834c5d28ab9c36d5`；真实 packaged `app.asar` 证据仍为 alpha.42。
...
- alpha.48 source 与最新 alpha.42 packaged smoke 均强制应用内外部验证并 PASS；packaged 第二进程队列恢复、制品/输出树哈希证据只属于 alpha.42。两者均保持 Electron sandbox，未使用 `--no-sandbox` 作为证据。
...
- Windows alpha 资源门禁：**PASS**。
  - alpha.48 源码 loose 资源：96 个文件 / 2,158,481 字节，manifest `267db60a…fc06`、anchor `e1f4718d…36d5`；最新真实 ASAR/packaged 全树仍为 alpha.42；
...
1. alpha.48 已完成账号 → 网站撤销设备 → 桌面显式刷新降为 Free 的无真实秘密仓库内纵向链；下一项转向标准更新 transport 的离线契约与假响应闭环，保持默认零网络和项目版本不静默升级；

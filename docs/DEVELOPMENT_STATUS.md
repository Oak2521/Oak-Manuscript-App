...
- 本地检查点标签：`chatgpt-v0.1.0-alpha.13`；已有未签名 Windows x64 NSIS/ZIP 与真实 packaged 证据，不代表正式发行
...
- 当前内置标准为 `oak-standards 2.0.0` / `oak-rules 2.0.0`（release sequence 2）：35 条规则、6 个白名单机械 fixer；alpha.5 新增默认引用解析政策，alpha.6—alpha.13 未改变标准内容或自动修复白名单。
...
| P0：Node + Python 统一测试 | **完成（最新统一回归通过）** | alpha.13 最终计数见下方“最新测试基线”；0 失败 |
| P0：真实桌面 UI 冒烟 | **完成（alpha.13 packaged PASS）** | 隐藏真实打包 EXE 完成 DOCX + EPUB 全闭环并强制运行 EpubCheck/Ace；各 4 次检查、1 个批次、3 个检查点且原稿哈希不变 |
...
| Windows NSIS / ZIP | **完成（未签名 alpha）** | 三归档验哈希导入；builder 独立全树锁成立；alpha.13 NSIS/ZIP 已生成并复验 |
| Windows 发布制品证据 | **完成（alpha.13）** | SHA256SUMS + canonical manifest 与真实 NSIS/ZIP 交叉复验通过 |
...
- 最终统一 `npm test`：**PASS，退出码 0，墙钟 157.8 秒**；Node 310/303/0/7（3.236 秒），Python 351/0 failures/0 errors/3 skipped（104.469 秒）。跳过项不计作通过。
- 资源信任专项已纳入 Node 全量：源码清单/锚点一致、loose 应用与四类运行资源漂移、额外文件/平台替换、真实 `app.asar` 读取和启动顺序均通过；`verify:resource-trust` PASS，应用清单 SHA-256 `1117ec70c3c7fd2d6ef64d48f71bdf42ced384c7db571cc10f509363857c4397`，锚点 SHA-256 `89d1739999d0000050d13e7fbedcfb8dde8f9c57132d8d80d2681b8148a22179`。
...
- alpha.13 packaged smoke 强制应用内外部验证，不再依赖调用者设置环境变量；缺陷 EPUB 的 EpubCheck/Ace 均产生可解析失败结果。
...
- 隐藏 `npm run smoke:packaged:win`：**SMOKE-RESULT PASS**；最新输出 `out/packaged-smoke/runs/ms4mqaar-f6f3d43d55a2726d/projects/`。两个项目均为 `app_version=0.1.0-alpha.13`、`source_hash_ok=true`、4 次检查、1 个修复批次、3 个检查点；当前问题 13/5、应用 fixes 5/2、PDF 251,655/178,394 字节；EPUB EpubCheck 为 5 error，Ace 为 8 项失败断言。退出后项目路径进程为 0。
...
- 真实 alpha.13 `app.asar`、9 项 fuse、loose 全树、Python/JRE/EpubCheck/Ace 与应用 smoke 已验证；packaged 资源中 `.pyc` 为 0。
- 最终 NSIS 189,944,918 字节 / SHA-256 `2a5ffcfa…e1851`；ZIP 233,758,073 字节 / `0ecbbcd5…5ddae`；发布证据复验通过。
...
- Windows Authenticode 和安装包签名尚未完成；alpha.13 制品仅供开发/内测；
...
- 2026-07-28：推进到 `0.1.0-alpha.13`；锁定 `@electron/fuses 2.1.3`，识别并固定 `WasmTrapHandlers`，新增严格 afterPack 全 9 项写入/回读和未来 fuse fail-closed；真实 Windows NSIS/ZIP、packaged smoke、发布摘要与全量回归通过；仍未签名。

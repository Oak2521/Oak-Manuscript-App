...
`npm run test:node` 运行本目录全部 `*.test.js`；`npm test` 依次运行 Node 与 `python/tests/` 全套测试。alpha.24 最终统一结果为 Node **413 total / 406 pass / 0 fail / 7 skip / 4.483 秒**，Python **351 项 / 0 失败 / 0 错误 / 3 跳过 / 104.577 秒**，墙钟 167.2 秒；跳过项不计作通过。准确环境证据以 `docs/TEST_REPORT.md` 为准。
...
- Windows 安装生命周期验收默认只读并精确绑定源码当前版本；专项测试覆盖 alpha.23/alpha.12 成功夹具、SemVer、NSIS x86 启动器与 x64 主程序、两开关授权门、零授权零启动/零输出、九阶段状态机、HKCU/快捷方式探针、持久化 sentinel、降级成功时 fail-closed 与 canonical 证据篡改。alpha.24 未打包，当前真实预检不会借用旧制品；
- Web 作业、HTTP handler 与 Supabase 适配测试位于三个 `web_*.test.js`：43 项覆盖五份 exact schema、可信主体、单任务同意、HTTPS/同源、Bearer/Cookie 分流、Cookie CSRF、畸形/重复 Authorization、无效 token、身份夹带、上传前门禁、六动作、主体隔离、幂等、TTL、删除失败、固定错误与无内容审计；handler 不监听端口，verifier 为替身，不冒充生产账号或零留存测试；

...
`npm run test:node` 运行本目录全部 `*.test.js`；`npm test` 依次运行 Node 与 `python/tests/` 全套测试。alpha.14 最终统一结果为 Node **323 total / 316 pass / 0 fail / 7 skip / 3.032 秒**，Python **351 项 / 0 失败 / 0 错误 / 3 跳过 / 103.078 秒**，墙钟 110.8 秒；跳过项不计作通过。准确环境证据以 `docs/TEST_REPORT.md` 为准。
...
- ASAR 内资源锚点、60 文件应用 loose 清单、四类平台锁绑定、真实 `app.asar` 读取、loose 伪锚点拒绝、启动前验证及 5 个可信根 blocker 的严格关闭条件；
...
- Windows 安装生命周期验收默认只读，精确绑定当前 alpha.14 与归档 alpha.12；覆盖 SemVer 预发布排序、NSIS x86 启动器与 x64 主程序、两开关授权门、零授权零启动/零输出、九阶段状态机、HKCU/快捷方式探针、持久化 sentinel、降级成功时 fail-closed 与清理、路径穿越和 canonical 证据篡改；
...
真实 EpubCheck/Ace 集成测试位于 `python/tests/test_external.py`。Ace 慢测默认跳过，需显式设置 `OAK_TEST_ACE=1` 且本机有受支持的 Chrome；好样本必须通过，缺陷样本必须失败。alpha.14 packaged smoke 强制通过受控链路运行缺陷样本并得到 EpubCheck 5 error / Ace 8 项失败断言；缺失或陈旧 EXE 不得复用。

alpha.14 最终隐藏 packaged smoke 为 **PASS**，运行根为 `out/packaged-smoke/runs/ms4oftya-1a2f6ac1ad56c4cd/projects/`；DOCX/EPUB 均先确认引用计划，各有 4 次检查、1 个修复批次、3 个检查点、`source_hash_ok=true`，且 EPUB 外部工具确实运行。实际安装生命周期仍未运行。

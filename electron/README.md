...
alpha.16 的 `resource-trust-anchor.json` 随代码进入 ASAR，绑定 64 个应用 loose 文件的 canonical 清单和目标平台四类运行锁；Windows Python/EpubCheck 锁再分别绑定 CPython/EpubCheck 官方来源证据摘要。packaged 启动在标准存储/窗口前验证全部树，且门禁只接受从真实 `app.asar` 读取的锚点。
...
当前 `0.1.0-alpha.16` 全量 Node/Python 回归、真实 packaged 全 9 fuse/ASAR/资源、CPython/EpubCheck provenance、强制外部验证 smoke 与发布证据均通过。最终运行根 `out/packaged-smoke/runs/ms4se5k4-0d1d2a33a1dd2017/projects/`；DOCX/EPUB 各 4 次检查、1 个修复批次、3 个检查点且原稿哈希不变，EPUB 实得 EpubCheck 5 error 与 Ace 8 项失败断言。CPython/EpubCheck 人工签署、未签名和其余 packaged sale blocker 仍有效。

...
当前源码的 `resource-trust-anchor.json` 绑定应用 loose 文件与目标平台运行锁；精确摘要以 `docs/TEST_REPORT.md` 为准。alpha.59 源码锚点为 112 文件 / 2,217,733 字节，manifest `7e25e075…b496`、anchor `0d055104…9def`；全仓库文本固定 LF，避免 Windows checkout 改写受信字节。最新真实 packaged 锚点、ASAR、fuse、资源门禁及隐藏 smoke 仍属于未签名 alpha.58。packaged 门禁继续从真实 ASAR 读取 production `package.json` 与 `oakReleaseIdentity`。

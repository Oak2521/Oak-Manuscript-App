...
| 文件 | 职责 |
|---|---|
| `main.js` | 创建沙箱窗口、注册固定 IPC、安装默认离线门禁、外链白名单、Provider 占位和 `app:info` |
...
`app:info` 返回 `appVersion`、规则包身份和 `packaged=app.isPackaged`。smoke 随后读取本次创建的 `project.json` 及其引用报告，核对 Python core 版本、check ID 和项目/检查记录/报告规则包一致；打包 smoke 还强制 `packaged=true`，避免误验旧 EXE、陈旧 core 或错误规则包。打包态 Python、JRE 或 Ace stage 失配时必须失败关闭，不得回退用户环境中的同名代码资源；Ace 还须通过受版本控制 full lock 的 Python 侧复核。当前 Ace 浏览器运行时仍明确依赖用户系统 Chrome，不能把上述资源约束误写成已捆绑受控浏览器。

当前 `0.1.0-alpha.2` 最新源码 smoke 已在沙箱外隐藏 Electron 中 PASS；项目严格写入 `out/source-smoke/projects/`，DOCX/EPUB 两个 `project.json` 均记录 `app_version=0.1.0-alpha.2` 和 `integrity.source_hash_ok=true`，PDF 分别为 258,394 与 161,830 字节。`release/` 尚无对应 EXE，因此不能声称打包 smoke 已通过。

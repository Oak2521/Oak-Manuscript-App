...
| `desktop-auth-config.js` / `desktop-application-login-provider.js` | 受信待配置门禁、系统浏览器 PKCE S256/state、随机 loopback callback、轮换 refresh 与账号生命周期 provider |
| `encrypted-auth-store.js` / `application-login-http-client.js` / `oak-account-token.js` | `OAKAUTH2` refresh-only safeStorage、固定 exchange/refresh/revoke HTTPS 客户端及本地 Ed25519 access-token 验证 |

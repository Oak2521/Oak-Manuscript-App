...
const EXACT_KEYS = Object.freeze([
  "schema_version", "config_type", "status", "update_endpoint",
]);
...
function canonicalHttpsUrl(value) {
  if (typeof value !== "string" || value.length < 9 || value.length > 2048) {
    throw new Error("桌面标准更新配置错误：update_endpoint 必须是 HTTPS URL");
  }
  let parsed;
  try { parsed = new URL(value); }
  catch { throw new Error("桌面标准更新配置错误：update_endpoint 非法"); }
  if (parsed.protocol !== "https:" || parsed.username || parsed.password || parsed.hash ||
      parsed.search || parsed.toString() !== value) {
    throw new Error("桌面标准更新配置错误：update_endpoint 必须是规范 HTTPS URL");
  }
  return value;
}
...
function validateDesktopStandardsUpdateConfig(value) {
  ...
  if (value.status === "pending_configuration") {
    if (value.update_endpoint !== null) {
      throw new Error("桌面标准更新配置错误：待配置状态不得携带半成品端点");
    }
    return Object.freeze({ ...value });
  }
  canonicalHttpsUrl(value.update_endpoint);
  return Object.freeze({ ...value });
}
